...
const CATALOG_ROUTE_DEFINITIONS: Record<CatalogNavKey, CatalogRouteDefinition> = {
  ...
  'all-bags': {
    ...
    categoryAliases: ['bags', 'bag', 'purse', 'purses', 'handbag', 'handbags'],
  },
  crossbody: {
...
export const HEADER_NAV_ITEMS: HeaderNavItem[] = HEADER_NAV_ORDER.map(navKey => ({
  id: navKey,
...
export const isLiveCatalogNavKey = (
  ...
  Boolean(
    navKey &&
      CATALOG_ROUTE_DEFINITIONS[navKey].availability === 'live'
  );
...
export const getCatalogNavKeyFromLocation = (
  pathname: string,
  search: string
) => getCatalogNavKeyFromPath(pathname) ?? getCatalogNavKeyFromLegacySearch(search);
...
export const getCatalogNavKeyFromTaxonomy = ({
  ...
  const subcategoryValues = getNormalizedValues(subcategoryName, subcategorySlug);
