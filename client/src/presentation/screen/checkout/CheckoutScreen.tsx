...
const CheckoutScreen: React.FC = () => {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      setOrderError(
        error?.message || 'Order could not be created. Please try again.'
      );
    } finally {
              ...
              <OrderItemsList>
                {cartItems.map(item => {
                  const itemPrice = parseFloat(
                    (item.product.price_new || item.product.price).replace(
                      /[^\d.-]/g,
                      ''
                    )
                  ) || 0;
                    ...
                    >
                      ...
                      <OrderItemDetails>
                        ...
                        >
                          <button
                            onClick={() =>
                              handleQuantityChange(item.bagId, item.quantity - 1)
                            }
