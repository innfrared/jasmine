...
const ProductScreen: React.FC<ProductScreenProps> = ({ initialProduct }) => {
  ...
  const { product, loading, error, mainImage, setMainImage, notFound } =
    useProductScreenModel(slug, productId, initialProduct);
  ...
  }, [product, setMainImage, getVariantImages]);
  ...
  const handleVariantSelect = (variantId: number) => {
    ...
    if (variant) {
      router.push(
        withLocale(
          locale,
          `/product/${buildProductSlug(variant.name, variantId)}`
        )
      );
    }
  };
  ...
  const allImages = getVariantImages.filter(img => !!img);
  const fallbackImage =
    getImageUrl(product?.variant_image || '') || '/assets/logo.png';
  const displayMainImage = mainImage || fallbackImage;
                  ...
                  <ColorOptions>
                    {product.variant_color_palette && (
                      ...
                      >
                        ...
                        <ColorName>
                          {product.variant_color_name || 'Current'}
                        </ColorName>
                      </ColorOption>
                    )}
                    {product.variants &&
