...
class AuthValidator {
    validateRegister(request) {
        ...
        if (typeof first_name !== 'string' || first_name.length == 0)
            return { message: "Name must be string" };
        // username rules
        if (typeof username !== 'string' || username.length < 6 || username.length > 20)
            return { message: "Username must be 6 and 20 characters long" };
        const usernameRegex = /^[a-z0-9._]+$/;
        if (!usernameRegex.test(username))
            return { message: 'Username can only contain a-z, 0-9, _, and .' };
        // email rules
